...
const SettingsPage = (props: Props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (response.status === 200) setFirstView(response.data?.firstView);
    };
    fetchData();
  }, [firstView]);
  ...
  const switchState = async (checked: boolean) => {
    ...
    if (view) {
      if (view.status >= 400) setFirstView(!checked);

      toast(
        view.status === 200 ? 'Successfully updated' : 'Something went wrong'
      );
    }
  };
  ...
  return (
    <div>
      ...
      <p className="text-muted-foreground">
        Enabling this will send you notifications when someone watched your
